...
func setupWorkspace(args Args, files []string) (dir string, cleanup func() error, err error) {
	...
	if os.Getenv("BAZEL_TESTING_USE_TMP_FOR_CACHE") != "" {
		...
		if err != nil {
			return "", cleanup, fmt.Errorf("error setting up 'output base dir' in hermetic test: %w", err)
		}
	...
	if err := os.MkdirAll(mainDir, 0777); err != nil {
		return "", cleanup, fmt.Errorf("error setting up workspace 'main' directory: %w", err)
	}
